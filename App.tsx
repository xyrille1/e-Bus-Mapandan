// Required by @supabase/supabase-js in React Native (URL API polyfill).
import "react-native-url-polyfill/auto";
// Register background GPS task at module level (must run before any component renders).
import "./src/features/driver/gps-broadcast/gpsBackgroundTask";
...
import { PinChangeScreen } from "./src/features/driver/auth/PinChangeScreen";
import { useDriverAuth } from "./src/features/driver/auth/useDriverAuth";
import {
  useRouteSelection,
  type DriverRoute,
} from "./src/features/driver/route-selection/useRouteSelection";
import { usePreShiftChecklist } from "./src/features/driver/trip-control/usePreShiftChecklist";
...
> = {
  "super-admin": {
    email: process.env.EXPO_PUBLIC_ADMIN_SUPER_EMAIL ?? "",
    password: process.env.EXPO_PUBLIC_ADMIN_SUPER_PASSWORD ?? "",
  },
  monitor: {
    email: process.env.EXPO_PUBLIC_ADMIN_MONITOR_EMAIL ?? "",
    password: process.env.EXPO_PUBLIC_ADMIN_MONITOR_PASSWORD ?? "",
  },
};
...
export default function App() {
  ...
  const [driverPhase, setDriverPhase] = useState<
    "route" | "checklist" | "shift"
  >("route");
  const [showPinChange, setShowPinChange] = useState(false);
  const driverAuth = useDriverAuth();
  const routeSelection = useRouteSelection();
  const preShift = usePreShiftChecklist({
    vehicleId: driverAuth.vehicleId ?? null,
    routeName: routeSelection.selectedRoute?.name ?? null,
  });
  ...
  // Refresh route list whenever driver authenticates
  useEffect(() => {
    if (driverAuth.isAuthenticated) {
      routeSelection.retry();
    }
  ...
  const handleMobileLogin = async () => {
    ...
    if (selectedRole === "driver") {
      // Delegates entirely to useDriverAuth — bcrypt compare runs server-side
      await driverAuth.login(loginIdentifier.trim(), loginSecret.trim());
      return;
    }
  ...
  const handleAdminLogin = () => {
    ...
    if (
      adminEmail.trim() !== selected.email ||
      adminPassword.trim() !== selected.password
    ) {
  ...
  // unused variable removed — canConfirm now comes from preShift.canConfirm
  if (isWeb) {
        ...
        <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.webLoginRoot}>
            <View style={styles.webLoginCard}>
              ...
              <Text style={styles.webLoginSubtitle}>
                LGU operations portal with role-based access
              </Text>
              ...
              <View style={styles.roleToggleRow}>
                <Pressable
                  style={[
                    styles.roleButton,
                    adminRole === "monitor"
                      ? styles.roleButtonActiveCommuter
                      : null,
                  ]}
                ...
                >
                  <Text
                    style={[
                      ...
                      adminRole === "monitor"
                        ? styles.roleButtonTextActiveCommuter
                        : null,
                    ]}
                  >
                    Monitor
                ...
                >
                  <Text
                    style={[
                      styles.roleButtonText,
                      adminRole === "super-admin"
                        ? styles.roleButtonTextActiveDriver
                        : null,
                    ]}
                  >
                    Super Admin
              ...
              <Text style={[styles.inputLabel, styles.inputLabelGap]}>
                Password
              </Text>
              <TextInput
                value={adminPassword}
              ...
              <Pressable
                style={[styles.loginButton, styles.loginButtonCommuter]}
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.webHeader}>
          <Text style={styles.webHeaderTitle}>
            Mapandan e-Bus Admin Web Portal
          </Text>
          <Text style={styles.webHeaderMeta}>
            Operations dashboard for dispatch and THC handover control (
            {adminRole})
          </Text>
          <Pressable
  ...
  const isEffectivelyAuthenticated =
    selectedRole === "driver" ? driverAuth.isAuthenticated : isAuthenticated;
        ...
        <View style={styles.loginRoot}>
          <View
            style={[
              styles.loginHero,
              selectedRole === "driver"
                ? styles.loginHeroDriver
                : styles.loginHeroCommuter,
            ]}
          >
            <View style={styles.loginBrandMark}>
              <Text style={styles.loginBrandMarkText}>
                {selectedRole === "driver" ? "D" : "C"}
              </Text>
            </View>
            <Text style={styles.loginBrandTitle}>
              {selectedRole === "driver" ? "TransitOps" : "TransitGo"}
            </Text>
            <Text style={styles.loginBrandSubtitle}>
              {selectedRole === "driver"
                ? "Driver Portal 2026"
                : "Commuter Portal 2026"}
            </Text>
          </View>

          <View style={styles.loginPanel}>
            <Text style={styles.loginPanelTitle}>Welcome back</Text>
            <Text style={styles.loginPanelSubtitle}>
              Choose role and sign in to continue
            </Text>
            ...
            <Text style={styles.inputLabel}>
              {selectedRole === "driver" ? "Employee ID" : "Email or phone"}
            </Text>
            <TextInput
              value={loginIdentifier}
              onChangeText={setLoginIdentifier}
              placeholder={
                selectedRole === "driver" ? "DRV-4821" : "you@example.com"
              }
              placeholderTextColor={colors.ink4}
              style={styles.inputField}
            />
            ...
            <Pressable
              style={[
                styles.loginButton,
                selectedRole === "driver"
                  ? styles.loginButtonDriver
                  : styles.loginButtonCommuter,
              ]}
              onPress={handleMobileLogin}
              disabled={selectedRole === "driver" && driverAuth.isLoading}
            >
              <Text style={styles.loginButtonText}>
                {selectedRole === "driver" && driverAuth.isLoading
                  ? "Signing in…"
                  : "Sign In"}
              </Text>
            </Pressable>

            {(selectedRole === "driver" ? driverAuth.error : loginError) ? (
  ...
  if (selectedRole === "driver" && driverPhase === "route") {
    return (
      <SafeAreaView style={styles.safeArea}>
        <StatusBar style="dark" />
          ...
          <ScrollView contentContainerStyle={{ padding: spacing.md }}>
            <View style={styles.profileCard}>
              ...
              <Text style={styles.profileSubtitle}>
                Choose the route assigned to your vehicle.
              </Text>

              {routeSelection.isLoading ? (
                <Text style={styles.loginHint}>Loading routes...</Text>
                ...
                <>
                  ...
                  <Pressable
                    ...
                    onPress={routeSelection.retry}
                  >
                    <Text style={styles.ctaButtonText}>Retry</Text>
                  </Pressable>
                </>
              ) : routeSelection.routes.length === 0 ? (
                <>
                  <Text style={styles.loginHint}>
                    No routes assigned to your vehicle. Contact dispatch.
                  </Text>
                  <Pressable
                    style={styles.ctaButton}
                    onPress={routeSelection.retry}
                ...
                routeSelection.routes.map((route: DriverRoute) => (
                  <Pressable
                    key={route.id}
                    style={[
                      styles.listRow,
                      routeSelection.selectedRoute?.id === route.id
                        ? styles.selectedRow
                        : null,
                    ]}
                    onPress={() => routeSelection.selectRoute(route)}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={styles.listMain}>{route.name}</Text>
                      {route.shortCode ? (
                        <Text style={styles.listMeta}>
                          {route.shortCode} • {route.stationCount} stops
                        </Text>
                      ) : (
                        <Text style={styles.listMeta}>
                    ...
                    {routeSelection.selectedRoute?.id === route.id ? (
                      <Text style={styles.selectedRowText}>Selected</Text>
                    ) : null}
                  </Pressable>
                ))
              )}
              ...
              <Pressable
                style={[
                  styles.loginButton,
                  styles.loginButtonDriver,
                  !routeSelection.selectedRoute ? styles.disabledButton : null,
                ]}
                disabled={!routeSelection.selectedRoute}
                onPress={() => {
                  if (routeSelection.confirmSelection()) {
                    setDriverPhase("checklist");
                  }
                }}
              >
  ...
  if (selectedRole === "driver" && driverPhase === "checklist") {
    return (
      <SafeAreaView style={styles.safeArea}>
        <StatusBar style="dark" />
        <View style={styles.onboardingRoot}>
          <View style={styles.profileCard}>
            ...
            <Text style={styles.profileSubtitle}>
              Confirm all items before starting GPS broadcast.
            </Text>

            <View style={styles.listRow}>
              <Text style={styles.listMain}>Route</Text>
              <Text style={[styles.listMeta, { color: colors.green }]}>
                {routeSelection.selectedRoute?.name ?? "None"}
              </Text>
            </View>
            ...
            <Pressable
              style={[
                styles.listRow,
                preShift.vehicleIdConfirmed ? styles.selectedRow : null,
              ]}
              onPress={preShift.confirmVehicleId}
            >
              <Text style={styles.listMain}>
                Vehicle ID ({driverAuth.vehicleId})
              </Text>
              <Text
                style={[
                  styles.listMeta,
                  preShift.vehicleIdConfirmed ? styles.selectedRowText : null,
                ]}
              >
                {preShift.vehicleIdConfirmed ? "Confirmed" : "Tap to confirm"}
              </Text>
            </Pressable>
            ...
            <Pressable
              style={[
                styles.listRow,
                preShift.routeConfirmed ? styles.selectedRow : null,
              ]}
              onPress={preShift.confirmRoute}
            >
              <Text style={styles.listMain}>Route assignment reviewed</Text>
              <Text
                style={[
                  styles.listMeta,
                  preShift.routeConfirmed ? styles.selectedRowText : null,
                ]}
              >
                {preShift.routeConfirmed ? "Confirmed" : "Tap to confirm"}
              </Text>
            </Pressable>

            <Pressable
              style={styles.listRow}
              onPress={preShift.refreshGpsSignal}
              disabled={preShift.isCheckingGps}
            >
              <Text style={styles.listMain}>GPS signal</Text>
              <Text
                style={[
                  styles.listMeta,
                  preShift.gpsSignalStrength === "good"
                    ? { color: colors.green, fontWeight: "800" }
                    : preShift.gpsSignalStrength === "weak"
                      ? { color: colors.amber }
                      : null,
                ]}
              >
                {preShift.isCheckingGps
                  ? "Checking..."
                  : preShift.gpsSignalStrength === "good"
                    ? "Good"
                    : preShift.gpsSignalStrength === "weak"
            ...
            <Pressable
              style={[
                styles.loginButton,
                styles.loginButtonDriver,
                !preShift.canConfirm ? styles.disabledButton : null,
              ]}
              disabled={!preShift.canConfirm}
              onPress={() => setDriverPhase("shift")}
            >
    ...
    if (commuterTab === "routes") {
      return (
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileSubtitle}>
            Favorite and recent commute routes
          </Text>
          <View style={styles.listRow}>
            <View style={{ flex: 1 }}>
    ...
    if (commuterTab === "profile") {
      return (
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileSubtitle}>
            Commuter profile and preferences
          </Text>
  ...
  const renderDriverContent = () => {
    ...
    if (driverTab === "routes") {
      return (
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileSubtitle}>
            Active and upcoming trips for this shift
          </Text>
          <View style={styles.listRow}>
            <View style={{ flex: 1 }}>
    ...
    if (driverTab === "profile") {
      ...
      return (
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileSubtitle}>
            Vehicle: {driverAuth.vehicleId}
          </Text>
          <View style={styles.profileStatsRow}>
            <View style={styles.profileStat}>
              <Text style={styles.profileStatLabel}>Vehicle</Text>
              <Text style={styles.profileStatValue}>
                {driverAuth.vehicleId ?? "--"}
              </Text>
            </View>
          </View>
          <View style={{ gap: spacing.xs, marginTop: spacing.xs }}>
            <View style={styles.listRow}>
              <Text style={styles.listMain}>Active Route</Text>
              <Text style={[styles.listMeta, { color: colors.green }]}>
                {routeSelection.selectedRoute?.name ?? "None"}
              </Text>
            </View>
          ...
          >
            <Text style={[styles.listMain, { color: colors.blue }]}>
              Change PIN
            </Text>
            <Text style={[styles.listMeta, { color: colors.blue }]}>›</Text>
          </Pressable>
          <Pressable style={styles.signOutButton} onPress={signOut}>
            <Text style={styles.signOutButtonText}>Sign Out</Text>
          </Pressable>
    ...
    return (
      <DriverTripControlScreen
        vehicleId={driverAuth.vehicleId!}
        routeId={routeSelection.selectedRoute!.id}
        routeName={routeSelection.selectedRoute!.name}
        driverId={driverAuth.driverId!}
        onShiftCompleted={() => setDriverPhase("route")}
      />
    );
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.appTopbar}>
        ...
        <View
          style={[
            styles.roleChip,
            selectedRole === "driver"
              ? styles.roleChipDriver
              : styles.roleChipCommuter,
          ]}
        >
          <Text
            style={[
              styles.roleChipText,
              selectedRole === "driver"
                ? styles.roleChipTextDriver
                : styles.roleChipTextCommuter,
            ]}
          >
      ...
      <View style={styles.tabbar}>
        <Pressable
          style={[
            styles.tabItem,
            (selectedRole === "driver" ? driverTab : commuterTab) === "home"
        ...
        <Pressable
          style={[
            styles.tabItem,
            (selectedRole === "driver" ? driverTab : commuterTab) === "map"
              ? styles.tabItemActive
              : null,
          ]}
          onPress={() => {
        ...
        <Pressable
          style={[
            styles.tabItem,
            (selectedRole === "driver" ? driverTab : commuterTab) === "routes"
              ? styles.tabItemActive
              : null,
          ]}
          onPress={() => {
        ...
        <Pressable
          style={[
            styles.tabItem,
            (selectedRole === "driver" ? driverTab : commuterTab) === "profile"
              ? styles.tabItemActive
              : null,
          ]}
          onPress={() => {
        ...
        >
          <Text
            style={[
              styles.tabLabel,
              (selectedRole === "driver" ? driverTab : commuterTab) ===
              "profile"
                ? styles.tabLabelActive
                : null,
            ]}
